...
// ── Crear tabla si no existe (solo al arrancar, no en cada request) ──
  ...
  try {
    ...
    if (req.file) {
      const result = await subirACloudinary(
        req.file.buffer,
        `prod_${req.params.barberiaId}_${Date.now()}`,
        "productos"
      );
...
router.put("/:barberiaId/:id", verificarToken, upload.single("imagen"), async (req, res) => {
  ...
  try {
    ...
    if (req.file) {
      const result = await subirACloudinary(
        req.file.buffer,
        `prod_${req.params.barberiaId}_${req.params.id}`,
        "productos"
      );
      imagen_url = result.secure_url;
    }
...
router.delete("/:barberiaId/:id", verificarToken, async (req, res) => {
  ...
  try {
    const [rows] = await db.query(
      "SELECT imagen_url FROM productos_barberia WHERE id=? AND id_barberia=?",
      [req.params.id, req.params.barberiaId]
    );
    if (rows.length && rows[0].imagen_url) {
      await eliminarDeCloudinary(rows[0].imagen_url, "productos");
    }
    await db.query(
      "DELETE FROM productos_barberia WHERE id=? AND id_barberia=?",
      [req.params.id, req.params.barberiaId]
    );
